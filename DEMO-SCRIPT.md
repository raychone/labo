...
3. Open `Acasă` and show the Manager, Recepție and Tehnician panels, their primary actions, and permission-driven navigation groups. Point out that the Tehnician dashboard is populated by available work, owned work, attention items and next actions.
...
28. Open `Centru operațional` and show wrapped tabs, the filter bar, grouped row metadata and the absence of billing labels for non-financial users.
29. Open `Facturare`, select the current month and show active company header, overview counters, billable works, proformas, invoices, receivables/restanțe and ambiguous legacy read-only review.
...
33. Select an invoice with balance and open `Înregistrează încasare`; show the manual payment modal and explain that it records external evidence only.
34. Search `CH-2026-001` and `OP-DEMO-001`, then show the payments tab.
35. Open the print route for a billing document, switch from document view to attachment view and use browser print preview.
36. Show the month registry and CSV export.
37. Open Centru operațional and show READY preparation groups with active delivery context.
38. Open Livrările mele as Manager/Logistică and show demo deliveries, including planned, assigned, in transit, delivered, failed and unassigned.
39. Filter Nereușite and show the failed delivery.
40. Open delivered delivery `DLV-2026-DEMO-07` or `DLV-2026-DEMO-08`, show “Confirmare internă de primire”, “Deschide dovada”, signature display and proof print route.
41. Open `DLV-2026-DEMO-11` and show the manager override proof without signature.
42. Use demo quick access to enter as Curier and open Livrările mele.
43. Show that the courier sees only own deliveries and no pricing/payment totals.
44. Open Scanare as Curier and resolve `WO-2026-900030`; show the delivery card and “Confirmă predarea”.
45. Open the in-transit delivery and demonstrate the signature modal: recipient, works, signature canvas, confirmation checkbox and “Confirmă predarea”.
46. Explain that the proof is internal operational evidence, not a qualified electronic signature, and that no money, POS, GPS, photos or file upload is involved.
47. Verify `/health`.

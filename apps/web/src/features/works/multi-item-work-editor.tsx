...
export function MultiItemWorkEditor({
  ...
  const shadeCatalog = selectedWorkType?.allowedShades === null || selectedWorkType?.allowedShades === undefined
  ...
  const shadeOptions = useMemo(() => {
    const values = shade && !shadeCatalog.includes(shade as (typeof WORK_SHADE_OPTIONS)[number]) ? [...shadeCatalog, shade] : shadeCatalog;
    return filterEditorOptions(values.map((value) => ({ label: value, secondary: undefined, value })), shade ? "" : shadeSearch);
  }, [selectedWorkType?.allowedShades, shade, shadeCatalog, shadeSearch]);
